...
bool NFmiFastQueryInfo::GetValuesPartial(size_t startIndex,
                                                ...
                                                std::vector<float> &values) const
{
  if(itsRefRawData && itsRefRawData->GetValuesPartial(startIndex, rowCount, rowStep, columnCount, columnStep, values))
  {
    DoSubParamConversions(values, __FUNCTION__);
    return true;
  }
...
void NFmiFastQueryInfo::DoSubParamConversions(std::vector<float>& values, std::string callingFunctionName) const
{
  if (IsSubParamUsed())
  {
    if (itsCombinedParamParser)
    {
      ...
      throw Fmi::Exception(BCP, callingFunctionName + ": CombinedParamParser missing!");
  }
}
                                                                       ...
                                                                       NFmiTimeBag &theTimes)
{
  try
  {
    ...
    else  // esim. hybridi datassa on levelit usein ylhäältä alas, joten data pitää täyttää
          // toisin päin
    {
      theInfo.LastLevel();  // huom! ei ole ResetLastLevel-metodia, vään tämä on FirstLevel:in
                            // vastin pari
                                                                ...
                                                                NFmiTimeBag &theWantedTimes)
{
  try
  {
    ...
    NFmiDataMatrix<float> values(theWantedTimes.GetSize(), theHeights.size(), kFloatMissing);

    // xnumberissa pitäisi olla poikkileikkaus pisteiden määrä ja ynumberissa haluttujen korkeuksien
    // määrä
    ...
    const std::vector<NFmiMetTime> &thePointTimes)
{
  try
  {
    ...
    // Ao. koodi vaatii samanpituiset paikka- ja aikavektorit, taytetaan toinen niista
    // tarvittaessa.
    //
...
NFmiDataMatrix<float> NFmiFastQueryInfo::GridValues(const NFmiGrid &theWantedGrid,
                                   const NFmiMetTime &theInterpolatedTime)
{
  return GridValues(theWantedGrid,
             theInterpolatedTime,
             180l);
}
    ...
    const NFmiDataMatrix<NFmiLocationCache> &theLocationCache)
{
  try
  {
    ...
    if (!cropNativeGrid)
    {
      ...
      throw Fmi::Exception(BCP,
                           "NFmiFastQueryInfo::LandscapeInterpolatedValuesDewPoint: Dewpoint and "
                           "temperature matrix "
                           "dimensions are not equal");

    if ((nx == 0) || (ny == 0) || (humidityMatrix.NX() != nx) || (humidityMatrix.NY() != ny))
      throw Fmi::Exception(BCP,
                           "NFmiFastQueryInfo::LandscapeInterpolatedValuesDewPoint: Humidity and "
                           "temperature matrix "
                           "dimensions are not equal");
    ...
    const NFmiDataMatrix<float> &maskMatrix)
{
  try
  {
    ...
    if (ngx == 0 || ngy == 0 || demMatrix.NX() != ngx || demMatrix.NY() != ngy ||
      ...
      throw Fmi::Exception(BCP,
                           "NFmiFastQueryInfo::LandscapeInterpolatedValues: Gridpoint, dem "
                           "and waterflag matrix "
                           "dimensions are not equal");
    ...
    const NFmiDataMatrix<float> &theDEMMatrix, const NFmiDataMatrix<bool> &theWaterFlagMatrix)
{
  try
  {
    ...
    if ((ident != kFmiTemperature) && (ident != kFmiDewPoint))
      throw Fmi::Exception(BCP,
                           "NFmiFastQueryInfo::LandscapeValues: Can only be used for "
